...
import 'purchase_rfq_details_sheet.dart';

class PurchaseRfqsScreen extends StatefulWidget {
  final bool showAppBar;
  const PurchaseRfqsScreen({super.key, this.showAppBar = true});

  @override
  State<PurchaseRfqsScreen> createState() => _PurchaseRfqsScreenState();
}

class _PurchaseRfqsScreenState extends State<PurchaseRfqsScreen> {
  bool _loading = true;
  ...
  Color _getStatusColor(String status) {
    switch (status.toLowerCase()) {
      ...
      case 'replied': return Colors.orange;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          final result = await Navigator.push(context, MaterialPageRoute(builder: (c) => const PurchaseRfqFormScreen()));
          if (result == true) _fetchRfqs();
        },
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                _buildFilterChip('Converted PO', 'converted'),
              ],
            ),
          ),
      ...
      return SliverFillRemaining(
        child: Center(
          child: Column(
            ...
            children: [
              Icon(Icons.request_quote_rounded, size: 64, color: context.textSecondary.withOpacity(0.2)),
              const SizedBox(height: 16),
              Text("No RFQs found", style: TextStyle(fontFamily: 'Outfit', color: context.textSecondary)),
            ],
          ),
        ),
      );
  ...
  Widget _buildRfqCard(Map<String, dynamic> rfq) {
    final status = rfq['status'] ?? 'Draft';
    final statusColor = _getStatusColor(status);
    final date = DateTime.tryParse(rfq['date']?.toString() ?? '') ?? 
                 DateTime.tryParse(rfq['created_at']?.toString() ?? '') ?? 
                 DateTime.now();
    final vendorName = (rfq['vendor'] != null && rfq['vendor']['name'] != null) 
        ? rfq['vendor']['name'].toString() 
        : 'Open RFQ';
              ...
              Row(
                ...
                children: [
                  Container(
                    ...
                    child: Text(rfq['rfq_number'] ?? '#---', style: const TextStyle(fontFamily: 'Outfit', fontWeight: FontWeight.bold, fontSize: 13, color: AppColors.primaryBlue)),
                  ),
                  Container(
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Icon(Icons.forum_outlined, color: context.textSecondary.withOpacity(0.5), size: 22),
                  ),
