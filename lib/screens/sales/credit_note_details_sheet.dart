...
class _CreditNoteDetailsSheetState extends State<CreditNoteDetailsSheet> {
  ...
  Future<void> _refreshData() async {
    try {
      final updatedCN = await Supabase.instance.client
          .from('sales_credit_notes')
          .select('*, customer:customers(name)')
          .eq('id', _creditNote['id'])
          .single();
      
      if (mounted) {
        setState(() {
          _creditNote = updatedCN;
        });
        await _fetchItems();
        widget.onRefresh();
      }
    } catch (e) {
      debugPrint("Error refreshing credit note: $e");
    }
  }

  Future<void> _archiveCN() async {
    final confirm = await _showConfirmDialog(
      title: "Archive Credit Note",
      message: "Are you sure you want to archive this credit note? It will be moved to the archive section.",
  ...
  Future<void> _deleteCN() async {
    final confirm = await _showConfirmDialog(
      title: "Delete Permanently",
      message: "WARNING: This action cannot be undone. Are you sure you want to delete this credit note forever?",
  ...
  Widget _buildActions() {
    return Column(
      children: [
        ...
        const Divider(),
        const SizedBox(height: 24),
        if (_creditNote['is_active'] != false)
          SizedBox(
  ...
  Widget _buildActionButton(IconData icon, String label, VoidCallback? onTap, {bool filled = false, bool isDestructive = false, Color? color, Color? textColor}) {
    final bgColor = color ?? (filled ? Colors.red : (isDestructive ? Colors.orange.withOpacity(0.1) : context.cardBg));
